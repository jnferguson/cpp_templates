...
class http_client_socket_t
{
	...
	public:
		...
		static bool get(std::string host, std::string uri, http_parameters_t& data, header_map_t& headers, http_response_t& resp, bool ssl = false, bool verify = false);
		static bool post(std::string host, std::string uri, http_parameters_t& data, header_map_t& headers, http_response_t& resp, http_content_type_t content_type = http_content_type_t::CONTENT_TYPE_MULTIPART_FORM_DATA, bool ssl = false, bool verify = false);
};
